...
class DataRepository @Inject constructor(
    ...
    override fun getMovieTrailer(movieId: Int): LiveData<Resource<List<MovieTrailerItemModel>>> {
        val result = MediatorLiveData<Resource<List<MovieTrailerItemModel>>>()

        result.addSource(remoteDataSource.getMovieTrailer(movieId)) { response ->
            when (response) {
                is ApiResponse.Success ->
                    appExecutors.diskIO().execute {
                        appExecutors.mainThread().execute {
                            result.value = Resource.Success(
                                DataMapper.MovieTrailer.mapResponsesToDomain(response.data)
                            )
                        }
                    }
                is ApiResponse.Empty -> appExecutors.mainThread().execute {
                    result.value = Resource.Success(
                        DataMapper.MovieTrailer.mapResponsesToDomain(
                            listOf()
                        )
                    )
                }
